...
abstract class Publicize_Base {
	...
	public function __construct() {
		...
		// Add images generated by Social Image Generator to the Open Graph tags on singular posts.
		add_filter( 'jetpack_open_graph_tags', array( $this, 'add_jetpack_social_og_images' ), 12, 1 ); // $priority = 12, to run after the Twitter_Cards class adds its tags.
	 ...
	 * @since 0.1.0
	 * @since-jetpack 6.7.0
	 * @deprecated 0.88.0-alpha Services and their connect URLs come from the `publicize/services` REST endpoint.
	 *
	 * @return array Always empty.
	 */
	public function get_available_service_data() {
		_deprecated_function( __METHOD__, 'publicize-0.88.0-alpha' );
	...
	public function add_jetpack_social_og_images( $tags ) {
		...
		if ( ! post_type_supports( get_post_type( $post_id ), 'publicize' ) ) {
			return $tags;
		}

		$opengraph_image = $this->get_social_opengraph_image( $post_id );
		...
		$needs_twitter_cards = class_exists( \Automattic\Jetpack\Post_Media\Twitter_Cards::class );
