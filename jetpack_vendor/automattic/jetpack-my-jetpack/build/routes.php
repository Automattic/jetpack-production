...
foreach ( $routes_by_page as $page_slug => $page_routes ) {
	...
	$global_name = 'jetpack_my_jetpack_' . $page_slug_underscore . '_routes_data';
 ...
 */
function jetpack_my_jetpack_register_page_routes( $page_routes, $register_function_name ) {
	...
	foreach ( $page_routes as $route ) {
		...
		if ( $route['has_content'] ) {
			...
			if ( file_exists( $content_asset_path ) ) {
				...
				$content_handle = 'jetpack-my-jetpack/routes/' . $route['name'] . '/content';
				$extension = '.min.js';
		...
		if ( $route['has_route'] ) {
			...
			if ( file_exists( $route_asset_path ) ) {
				...
				$route_handle = 'jetpack-my-jetpack/routes/' . $route['name'] . '/route';
				$extension = '.min.js';
				// Deregister first to override any previously registered version
				// (e.g., Core's default modules when running as a plugin).
...
// Page-specific route registration functions for my-jetpack-dashboard
/**
 * Register routes for my-jetpack-dashboard page (full-page mode).
 */
function jetpack_my_jetpack_register_my_jetpack_dashboard_page_routes() {
	global $jetpack_my_jetpack_my_jetpack_dashboard_routes_data;
	jetpack_my_jetpack_register_page_routes( $jetpack_my_jetpack_my_jetpack_dashboard_routes_data, 'jetpack_my_jetpack_register_my_jetpack_dashboard_route' );
}
add_action( 'my-jetpack-dashboard_init', 'jetpack_my_jetpack_register_my_jetpack_dashboard_page_routes' );

/**
 * Register routes for my-jetpack-dashboard page (wp-admin mode).
 */
function jetpack_my_jetpack_register_my_jetpack_dashboard_wp_admin_page_routes() {
	global $jetpack_my_jetpack_my_jetpack_dashboard_routes_data;
	jetpack_my_jetpack_register_page_routes( $jetpack_my_jetpack_my_jetpack_dashboard_routes_data, 'jetpack_my_jetpack_register_my_jetpack_dashboard_wp_admin_route' );
}
add_action( 'my-jetpack-dashboard-wp-admin_init', 'jetpack_my_jetpack_register_my_jetpack_dashboard_wp_admin_page_routes' );
