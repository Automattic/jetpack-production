<?php
/**
 * Page: my-jetpack-dashboard (wp-admin integrated)
...
// Global storage for my-jetpack-dashboard routes and menu items
global $jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_routes, $jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_menu_items;
$jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_routes     = array();
$jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_menu_items = array();

/**
 * Register a route for the my-jetpack-dashboard-wp-admin page.
 *
 ...
 */
function jetpack_my_jetpack_register_my_jetpack_dashboard_wp_admin_route( $path, $content_module = null, $route_module = null ) {
	global $jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_routes;
	...
	$jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_routes[] = $route;
}

/**
 * Register a menu item for the my-jetpack-dashboard-wp-admin page.
 ...
 */
function jetpack_my_jetpack_register_my_jetpack_dashboard_wp_admin_menu_item( $id, $label, $to, $parent_id = '' ) {
	global $jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_menu_items;
	...
	$jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the my-jetpack-dashboard-wp-admin page.
 *
 * @return array Array of route objects.
 */
function jetpack_my_jetpack_get_my_jetpack_dashboard_wp_admin_routes() {
	global $jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_routes;
	return $jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_routes ?? array();
}

/**
 * Get all registered menu items for the my-jetpack-dashboard-wp-admin page.
 *
 * @return array Array of menu item objects.
 */
function jetpack_my_jetpack_get_my_jetpack_dashboard_wp_admin_menu_items() {
	global $jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_menu_items;
	return $jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_menu_items ?? array();
}

/**
 * Preload REST API data for the my-jetpack-dashboard-wp-admin page.
 * Automatically called during page rendering.
 */
function jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_preload_data() {
...
/**
 * Enqueue scripts and styles for the my-jetpack-dashboard-wp-admin page.
 * Hooked to admin_enqueue_scripts.
 *
 * @param string $hook_suffix The current admin page.
 */
function jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_enqueue_scripts( $hook_suffix ) {
	// Check all possible ways this page can be accessed:
	// 1. Menu page via admin.php?page=my-jetpack-dashboard-wp-admin (plugin)
	// 2. Direct file via my-jetpack-dashboard.php (Core) - screen ID will be 'my-jetpack-dashboard'
	$current_screen = get_current_screen();
	$is_our_page = (
		( isset( $_GET['page'] ) && 'my-jetpack-dashboard-wp-admin' === $_GET['page'] ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		( $current_screen && 'my-jetpack-dashboard' === $current_screen->id )
	);
	...
	/**
	 * Fires when the my-jetpack-dashboard admin page is initialized so extensions can register routes and menu items.
	 */
	do_action( 'my-jetpack-dashboard-wp-admin_init' );

	// Preload REST API data
	jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_preload_data();

	// Get all registered routes
	$routes = jetpack_my_jetpack_get_my_jetpack_dashboard_wp_admin_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'my-jetpack-dashboard-wp-admin-prerequisites', '', $asset['dependencies'], $asset['version'], true );

		$init_modules = ["@jetpack-my-jetpack/init"];
...
JS;
		wp_add_inline_script(
			'my-jetpack-dashboard-wp-admin-prerequisites',
			sprintf(
				'( %s )( %s, %s, %s );',
				$init_js_function,
				wp_json_encode( 'my-jetpack-dashboard-wp-admin-app', JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
		...
		wp_register_style( 'my-jetpack-dashboard-wp-admin-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for my-jetpack-dashboard-wp-admin module
		...
		// Add init modules as static dependencies
			$boot_dependencies[] = array( 'import' => 'static', 'id' => '@jetpack-my-jetpack/init' );
		 ...
		 * my-jetpack-dashboard-wp-admin page.
		 *
		...
		$boot_dependencies = apply_filters(
			'my-jetpack-dashboard-wp-admin_boot_dependencies',
			$boot_dependencies
		);

		// Dummy script module to ensure dependencies are loaded
		wp_register_script_module(
			'my-jetpack-dashboard-wp-admin',
			$build_constants['build_url'] . 'pages/my-jetpack-dashboard/loader.js',
			$boot_dependencies
		);

		// Enqueue the boot scripts and styles
		wp_enqueue_script( 'my-jetpack-dashboard-wp-admin-prerequisites' );
		wp_enqueue_script_module( 'my-jetpack-dashboard-wp-admin' );
		wp_enqueue_style( 'my-jetpack-dashboard-wp-admin-prerequisites' );
	}
}

/**
 * Render the my-jetpack-dashboard-wp-admin page.
...
function jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_render_page() {
	...
	<style>
		...
		body.js #wpbody-content > div:not(#my-jetpack-dashboard-wp-admin-app):not(#screen-meta) {
			display: none;
		}
		body.js #wpfooter {
			display: none;
	...
	?>
	<div id="my-jetpack-dashboard-wp-admin-app" class="boot-layout-container"></div>
	<?php
}

// Hook the enqueue function to admin_enqueue_scripts
add_action( 'admin_enqueue_scripts', 'jetpack_my_jetpack_my_jetpack_dashboard_wp_admin_enqueue_scripts' );
