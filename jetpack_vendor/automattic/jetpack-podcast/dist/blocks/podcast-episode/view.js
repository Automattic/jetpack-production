/******/ (() => { // webpackBootstrap
/*!********************************************!*\
  !*** ./src/blocks/podcast-episode/view.ts ***!
  \********************************************/
/* wp:polyfill */
...
const awaitingMetadata = new WeakSet();
const applyPendingSeek = media => {
...
const seekAndPlay = (media, seconds) => {
...
const wireEpisode = root => {
  const media = root.querySelector('.jetpack-podcast-episode__audio, .jetpack-podcast-episode__video');
  if (!media) {
    return;
  }
  root.addEventListener('click', event => {
    const button = event.target?.closest('button[data-start-time]');
    if (!button) {
      return;
    }
...
const init = () => {
  document.querySelectorAll('.wp-block-jetpack-podcast-episode').forEach(wireEpisode);
};
