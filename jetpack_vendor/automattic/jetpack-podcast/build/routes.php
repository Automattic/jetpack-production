...
foreach ( $routes_by_page as $page_slug => $page_routes ) {
	...
	$global_name = 'jetpack_podcast_' . $page_slug_underscore . '_routes_data';
 ...
 */
function jetpack_podcast_register_page_routes( $page_routes, $register_function_name ) {
	...
	foreach ( $page_routes as $route ) {
		...
		if ( $route['has_content'] ) {
			...
			if ( file_exists( $content_asset_path ) ) {
				...
				$content_handle = 'jetpack-podcast/routes/' . $route['name'] . '/content';
		...
		if ( $route['has_route'] ) {
			...
			if ( file_exists( $route_asset_path ) ) {
				...
				$route_handle = 'jetpack-podcast/routes/' . $route['name'] . '/route';
				$extension = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '.js' : '.min.js';
				// Deregister first to override any previously registered version
				// (e.g., Core's default modules when running as a plugin).
...
// Page-specific route registration functions for jetpack-podcast-dashboard
/**
 * Register routes for jetpack-podcast-dashboard page (full-page mode).
 */
function jetpack_podcast_register_jetpack_podcast_dashboard_page_routes() {
	global $jetpack_podcast_jetpack_podcast_dashboard_routes_data;
	jetpack_podcast_register_page_routes( $jetpack_podcast_jetpack_podcast_dashboard_routes_data, 'jetpack_podcast_register_jetpack_podcast_dashboard_route' );
}
add_action( 'jetpack-podcast-dashboard_init', 'jetpack_podcast_register_jetpack_podcast_dashboard_page_routes' );

/**
 * Register routes for jetpack-podcast-dashboard page (wp-admin mode).
 */
function jetpack_podcast_register_jetpack_podcast_dashboard_wp_admin_page_routes() {
	global $jetpack_podcast_jetpack_podcast_dashboard_routes_data;
	jetpack_podcast_register_page_routes( $jetpack_podcast_jetpack_podcast_dashboard_routes_data, 'jetpack_podcast_register_jetpack_podcast_dashboard_wp_admin_route' );
}
add_action( 'jetpack-podcast-dashboard-wp-admin_init', 'jetpack_podcast_register_jetpack_podcast_dashboard_wp_admin_page_routes' );
