<?php
/**
 * Page: jetpack-podcast-dashboard (wp-admin integrated)
...
global $jetpack_podcast_jetpack_podcast_dashboard_wp_admin_routes, $jetpack_podcast_jetpack_podcast_dashboard_wp_admin_menu_items;
$jetpack_podcast_jetpack_podcast_dashboard_wp_admin_routes     = array();
$jetpack_podcast_jetpack_podcast_dashboard_wp_admin_menu_items = array();

/**
 * Register a route for the jetpack-podcast-dashboard-wp-admin page.
 *
 ...
 */
function jetpack_podcast_register_jetpack_podcast_dashboard_wp_admin_route( $path, $content_module = null, $route_module = null ) {
	global $jetpack_podcast_jetpack_podcast_dashboard_wp_admin_routes;
	...
	$jetpack_podcast_jetpack_podcast_dashboard_wp_admin_routes[] = $route;
}

/**
 * Register a menu item for the jetpack-podcast-dashboard-wp-admin page.
 ...
 */
function jetpack_podcast_register_jetpack_podcast_dashboard_wp_admin_menu_item( $id, $label, $to, $parent_id = '' ) {
	global $jetpack_podcast_jetpack_podcast_dashboard_wp_admin_menu_items;
	...
	$jetpack_podcast_jetpack_podcast_dashboard_wp_admin_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the jetpack-podcast-dashboard-wp-admin page.
 *
 * @return array Array of route objects.
 */
function jetpack_podcast_get_jetpack_podcast_dashboard_wp_admin_routes() {
	global $jetpack_podcast_jetpack_podcast_dashboard_wp_admin_routes;
	return $jetpack_podcast_jetpack_podcast_dashboard_wp_admin_routes ?? array();
}

/**
 * Get all registered menu items for the jetpack-podcast-dashboard-wp-admin page.
 *
 * @return array Array of menu item objects.
 */
function jetpack_podcast_get_jetpack_podcast_dashboard_wp_admin_menu_items() {
	global $jetpack_podcast_jetpack_podcast_dashboard_wp_admin_menu_items;
	return $jetpack_podcast_jetpack_podcast_dashboard_wp_admin_menu_items ?? array();
}

/**
 * Preload REST API data for the jetpack-podcast-dashboard-wp-admin page.
 * Automatically called during page rendering.
 */
function jetpack_podcast_jetpack_podcast_dashboard_wp_admin_preload_data() {
...
/**
 * Enqueue scripts and styles for the jetpack-podcast-dashboard-wp-admin page.
 * Hooked to admin_enqueue_scripts.
 *
 * @param string $hook_suffix The current admin page.
 */
function jetpack_podcast_jetpack_podcast_dashboard_wp_admin_enqueue_scripts( $hook_suffix ) {
	// Check all possible ways this page can be accessed:
	// 1. Menu page via admin.php?page=jetpack-podcast-dashboard-wp-admin (plugin)
	// 2. Direct file via jetpack-podcast-dashboard.php (Core) - screen ID will be 'jetpack-podcast-dashboard'
	$current_screen = get_current_screen();
	$is_our_page = (
		( isset( $_GET['page'] ) && 'jetpack-podcast-dashboard-wp-admin' === $_GET['page'] ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		( $current_screen && 'jetpack-podcast-dashboard' === $current_screen->id )
	);
	...
	jetpack_podcast_jetpack_podcast_dashboard_wp_admin_preload_data();

	// Get all registered routes
	$routes = jetpack_podcast_get_jetpack_podcast_dashboard_wp_admin_routes();
...
JS;
		...
		wp_register_style( 'jetpack-podcast-dashboard-wp-admin-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for jetpack-podcast-dashboard-wp-admin module
		 ...
		 * jetpack-podcast-dashboard-wp-admin page.
		 *
		...
		$boot_dependencies = apply_filters(
			'jetpack-podcast-dashboard-wp-admin_boot_dependencies',
			$boot_dependencies
		);

		// Dummy script module to ensure dependencies are loaded
		wp_register_script_module(
			'jetpack-podcast-dashboard-wp-admin',
			$build_constants['build_url'] . 'pages/jetpack-podcast-dashboard/loader.js',
			$boot_dependencies
		);

		// Enqueue the boot scripts and styles
		wp_enqueue_script( 'jetpack-podcast-dashboard-wp-admin-prerequisites' );
		wp_enqueue_script_module( 'jetpack-podcast-dashboard-wp-admin' );
		wp_enqueue_style( 'jetpack-podcast-dashboard-wp-admin-prerequisites' );
	}
}

/**
 * Render the jetpack-podcast-dashboard-wp-admin page.
 * Call this function from add_menu_page or add_submenu_page.
 * This renders within the normal WordPress admin interface.
 */
function jetpack_podcast_jetpack_podcast_dashboard_wp_admin_render_page() {
	?>
	<style>
		...
		body.js #wpbody-content > div:not(#jetpack-podcast-dashboard-wp-admin-app):not(#screen-meta) {
			display: none;
		}
		body.js #wpfooter {
			display: none;
	...
	?>
	<div id="jetpack-podcast-dashboard-wp-admin-app" class="boot-layout-container"></div>
	<?php
}
