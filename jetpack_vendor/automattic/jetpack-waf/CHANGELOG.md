...
- Connected the WAF UI to actually updating the IP block and allow lists when saving the settings.

### Fixed
- fixed database logging
- fixed issue where code for the waf package was executed if the module was disabled
- Fixed writing rules php files if the API request for getting up-to-date rules failes so that the internal functionality is kept in tact.
- We now sanitize the output generated by blocked requests, and only report the rule ID in the header response.
