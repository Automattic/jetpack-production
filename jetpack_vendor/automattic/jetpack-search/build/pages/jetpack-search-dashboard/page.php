...
global $jetpack_search_jetpack_search_dashboard_routes, $jetpack_search_jetpack_search_dashboard_menu_items;
$jetpack_search_jetpack_search_dashboard_routes     = array();
$jetpack_search_jetpack_search_dashboard_menu_items = array();

/**
 * Register a route for the jetpack-search-dashboard page.
 *
 ...
 */
function jetpack_search_register_jetpack_search_dashboard_route( $path, $content_module = null, $route_module = null ) {
	global $jetpack_search_jetpack_search_dashboard_routes;
	...
	$jetpack_search_jetpack_search_dashboard_routes[] = $route;
}

/**
 * Register a menu item for the jetpack-search-dashboard page.
 *
 ...
 */
function jetpack_search_register_jetpack_search_dashboard_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $jetpack_search_jetpack_search_dashboard_menu_items;
	...
	$jetpack_search_jetpack_search_dashboard_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the jetpack-search-dashboard page.
 *
 * @return array Array of route objects.
 */
function jetpack_search_get_jetpack_search_dashboard_routes() {
	global $jetpack_search_jetpack_search_dashboard_routes;
	return $jetpack_search_jetpack_search_dashboard_routes ?? array();
}

/**
 * Get all registered menu items for the jetpack-search-dashboard page.
 *
 * @return array Array of menu item objects.
 */
function jetpack_search_get_jetpack_search_dashboard_menu_items() {
	global $jetpack_search_jetpack_search_dashboard_menu_items;
	return $jetpack_search_jetpack_search_dashboard_menu_items ?? array();
}

/**
 * Preload REST API data for the jetpack-search-dashboard page.
 * Automatically called during page rendering.
 */
function jetpack_search_jetpack_search_dashboard_preload_data() {
...
/**
 * Render the jetpack-search-dashboard page.
 * Call this function from add_menu_page or add_submenu_page.
 */
function jetpack_search_jetpack_search_dashboard_render_page() {
	...
	/**
	 * Fires when the jetpack-search-dashboard page is initialized so extensions can register routes and menu items.
	 */
	do_action( 'jetpack-search-dashboard_init' );
	...
	jetpack_search_jetpack_search_dashboard_preload_data();

	// Get all registered routes and menu items
	$menu_items = jetpack_search_get_jetpack_search_dashboard_menu_items();
	$routes = jetpack_search_get_jetpack_search_dashboard_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_style( 'jetpack-search-dashboard-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for jetpack-search-dashboard module
		...
		$boot_dependencies = apply_filters(
			'jetpack-search-dashboard_boot_dependencies',
			$boot_dependencies
		);

		// Dummy script module to ensure dependencies are loaded
		wp_register_script_module(
			'jetpack-search-dashboard',
			$build_constants['build_url'] . 'pages/jetpack-search-dashboard/loader.js',
			$boot_dependencies
		);

		// Enqueue the boot scripts and styles
		wp_enqueue_script( 'jetpack-search-dashboard-prerequisites' );
		wp_enqueue_script_module( 'jetpack-search-dashboard' );
		wp_enqueue_style( 'jetpack-search-dashboard-prerequisites' );
	}
	...
	$hook_suffix = 'jetpack-search-dashboard';
	...
	<body class="jetpack-search-dashboard no-js">
	<?php
	// BEGIN see wp-admin/admin-header.php
	?>
	...
	?>
		...
		<div id="jetpack-search-dashboard-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
 ...
 */
function jetpack_search_jetpack_search_dashboard_intercept_render() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'jetpack-search-dashboard' === $_GET['page'] ) {
		...
		jetpack_search_jetpack_search_dashboard_render_page();
		exit;
	}
}

// Hook the interceptor to admin_init
add_action( 'admin_init', 'jetpack_search_jetpack_search_dashboard_intercept_render' );
