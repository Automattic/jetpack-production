...
foreach ( $routes_by_page as $page_slug => $page_routes ) {
	...
	$global_name = 'jetpack_search_' . $page_slug_underscore . '_routes_data';
 ...
 */
function jetpack_search_register_page_routes( $page_routes, $register_function_name ) {
	...
	foreach ( $page_routes as $route ) {
		...
		if ( $route['has_content'] ) {
			...
			if ( file_exists( $content_asset_path ) ) {
				...
				$content_handle = 'jetpack-search/routes/' . $route['name'] . '/content';
				$extension = '.min.js';
		...
		if ( $route['has_route'] ) {
			...
			if ( file_exists( $route_asset_path ) ) {
				...
				$route_handle = 'jetpack-search/routes/' . $route['name'] . '/route';
				$extension = '.min.js';
				// Deregister first to override any previously registered version
				// (e.g., Core's default modules when running as a plugin).
...
// Page-specific route registration functions for jetpack-search-dashboard
/**
 * Register routes for jetpack-search-dashboard page (full-page mode).
 */
function jetpack_search_register_jetpack_search_dashboard_page_routes() {
	global $jetpack_search_jetpack_search_dashboard_routes_data;
	jetpack_search_register_page_routes( $jetpack_search_jetpack_search_dashboard_routes_data, 'jetpack_search_register_jetpack_search_dashboard_route' );
}
add_action( 'jetpack-search-dashboard_init', 'jetpack_search_register_jetpack_search_dashboard_page_routes' );

/**
 * Register routes for jetpack-search-dashboard page (wp-admin mode).
 */
function jetpack_search_register_jetpack_search_dashboard_wp_admin_page_routes() {
	global $jetpack_search_jetpack_search_dashboard_routes_data;
	jetpack_search_register_page_routes( $jetpack_search_jetpack_search_dashboard_routes_data, 'jetpack_search_register_jetpack_search_dashboard_wp_admin_route' );
}
add_action( 'jetpack-search-dashboard-wp-admin_init', 'jetpack_search_register_jetpack_search_dashboard_wp_admin_page_routes' );
