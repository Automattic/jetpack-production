<?php
/**
 * Page: jetpack-ai-hub (wp-admin integrated)
...
// Global storage for jetpack-ai-hub routes and menu items
global $jetpack_plugin_jetpack_ai_hub_wp_admin_routes, $jetpack_plugin_jetpack_ai_hub_wp_admin_menu_items;
$jetpack_plugin_jetpack_ai_hub_wp_admin_routes     = array();
$jetpack_plugin_jetpack_ai_hub_wp_admin_menu_items = array();

/**
 * Register a route for the jetpack-ai-hub-wp-admin page.
 *
 ...
 */
function jetpack_plugin_register_jetpack_ai_hub_wp_admin_route( $path, $content_module = null, $route_module = null ) {
	global $jetpack_plugin_jetpack_ai_hub_wp_admin_routes;
	...
	$jetpack_plugin_jetpack_ai_hub_wp_admin_routes[] = $route;
}

/**
 * Register a menu item for the jetpack-ai-hub-wp-admin page.
 ...
 */
function jetpack_plugin_register_jetpack_ai_hub_wp_admin_menu_item( $id, $label, $to, $parent_id = '' ) {
	global $jetpack_plugin_jetpack_ai_hub_wp_admin_menu_items;
	...
	$jetpack_plugin_jetpack_ai_hub_wp_admin_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the jetpack-ai-hub-wp-admin page.
 *
 * @return array Array of route objects.
 */
function jetpack_plugin_get_jetpack_ai_hub_wp_admin_routes() {
	global $jetpack_plugin_jetpack_ai_hub_wp_admin_routes;
	return $jetpack_plugin_jetpack_ai_hub_wp_admin_routes ?? array();
}

/**
 * Get all registered menu items for the jetpack-ai-hub-wp-admin page.
 *
 * @return array Array of menu item objects.
 */
function jetpack_plugin_get_jetpack_ai_hub_wp_admin_menu_items() {
	global $jetpack_plugin_jetpack_ai_hub_wp_admin_menu_items;
	return $jetpack_plugin_jetpack_ai_hub_wp_admin_menu_items ?? array();
}

/**
 * Preload REST API data for the jetpack-ai-hub-wp-admin page.
 * Automatically called during page rendering.
 */
function jetpack_plugin_jetpack_ai_hub_wp_admin_preload_data() {
...
/**
 * Enqueue scripts and styles for the jetpack-ai-hub-wp-admin page.
 * Hooked to admin_enqueue_scripts.
 *
 * @param string $hook_suffix The current admin page.
 */
function jetpack_plugin_jetpack_ai_hub_wp_admin_enqueue_scripts( $hook_suffix ) {
	// Check all possible ways this page can be accessed:
	// 1. Menu page via admin.php?page=jetpack-ai-hub-wp-admin (plugin)
	// 2. Direct file via jetpack-ai-hub.php (Core) - screen ID will be 'jetpack-ai-hub'
	$current_screen = get_current_screen();
	$is_our_page = (
		( isset( $_GET['page'] ) && 'jetpack-ai-hub-wp-admin' === $_GET['page'] ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		( $current_screen && 'jetpack-ai-hub' === $current_screen->id )
	);
	...
	/**
	 * Fires when the jetpack-ai-hub admin page is initialized so extensions can register routes and menu items.
	 */
	do_action( 'jetpack-ai-hub-wp-admin_init' );

	// Preload REST API data
	jetpack_plugin_jetpack_ai_hub_wp_admin_preload_data();

	// Get all registered routes
	$routes = jetpack_plugin_get_jetpack_ai_hub_wp_admin_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'jetpack-ai-hub-wp-admin-prerequisites', '', $asset['dependencies'], $asset['version'], true );

		$init_modules = ["@jetpack-plugin/init"];
...
JS;
		wp_add_inline_script(
			'jetpack-ai-hub-wp-admin-prerequisites',
			sprintf(
				'( %s )( %s, %s, %s );',
				$init_js_function,
				wp_json_encode( 'jetpack-ai-hub-wp-admin-app', JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
		...
		wp_register_style( 'jetpack-ai-hub-wp-admin-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for jetpack-ai-hub-wp-admin module
		...
		// Add init modules as static dependencies
			$boot_dependencies[] = array( 'import' => 'static', 'id' => '@jetpack-plugin/init' );
		...
		$boot_dependencies = apply_filters(
			'jetpack-ai-hub-wp-admin_boot_dependencies',
			$boot_dependencies
		);

		// Dummy script module to ensure dependencies are loaded
		wp_register_script_module(
			'jetpack-ai-hub-wp-admin',
			$build_constants['build_url'] . 'pages/jetpack-ai-hub/loader.js',
			$boot_dependencies
		);

		// Enqueue the boot scripts and styles
		wp_enqueue_script( 'jetpack-ai-hub-wp-admin-prerequisites' );
		wp_enqueue_script_module( 'jetpack-ai-hub-wp-admin' );
		wp_enqueue_style( 'jetpack-ai-hub-wp-admin-prerequisites' );
	}
}

/**
 * Render the jetpack-ai-hub-wp-admin page.
 * Call this function from add_menu_page or add_submenu_page.
 * This renders within the normal WordPress admin interface.
 */
function jetpack_plugin_jetpack_ai_hub_wp_admin_render_page() {
	?>
	<style>
		...
		body.js #wpbody-content > div:not(#jetpack-ai-hub-wp-admin-app):not(#screen-meta) {
			display: none;
		}
		body.js #wpfooter {
			display: none;
	...
	?>
	<div id="jetpack-ai-hub-wp-admin-app" class="boot-layout-container"></div>
	<?php
}

// Hook the enqueue function to admin_enqueue_scripts
add_action( 'admin_enqueue_scripts', 'jetpack_plugin_jetpack_ai_hub_wp_admin_enqueue_scripts' );
