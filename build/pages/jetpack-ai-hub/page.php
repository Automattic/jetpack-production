...
global $jetpack_plugin_jetpack_ai_hub_routes, $jetpack_plugin_jetpack_ai_hub_menu_items;
$jetpack_plugin_jetpack_ai_hub_routes     = array();
$jetpack_plugin_jetpack_ai_hub_menu_items = array();

/**
 * Register a route for the jetpack-ai-hub page.
 *
 ...
 */
function jetpack_plugin_register_jetpack_ai_hub_route( $path, $content_module = null, $route_module = null ) {
	global $jetpack_plugin_jetpack_ai_hub_routes;
	...
	$jetpack_plugin_jetpack_ai_hub_routes[] = $route;
}

/**
 * Register a menu item for the jetpack-ai-hub page.
 *
 ...
 */
function jetpack_plugin_register_jetpack_ai_hub_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $jetpack_plugin_jetpack_ai_hub_menu_items;
	...
	$jetpack_plugin_jetpack_ai_hub_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the jetpack-ai-hub page.
 *
 * @return array Array of route objects.
 */
function jetpack_plugin_get_jetpack_ai_hub_routes() {
	global $jetpack_plugin_jetpack_ai_hub_routes;
	return $jetpack_plugin_jetpack_ai_hub_routes ?? array();
}

/**
 * Get all registered menu items for the jetpack-ai-hub page.
 *
 * @return array Array of menu item objects.
 */
function jetpack_plugin_get_jetpack_ai_hub_menu_items() {
	global $jetpack_plugin_jetpack_ai_hub_menu_items;
	return $jetpack_plugin_jetpack_ai_hub_menu_items ?? array();
}

/**
 * Preload REST API data for the jetpack-ai-hub page.
 * Automatically called during page rendering.
 */
function jetpack_plugin_jetpack_ai_hub_preload_data() {
...
/**
 * Render the jetpack-ai-hub page.
 * Call this function from add_menu_page or add_submenu_page.
 */
function jetpack_plugin_jetpack_ai_hub_render_page() {
	...
	/**
	 * Fires when the jetpack-ai-hub page is initialized so extensions can register routes and menu items.
	 */
	do_action( 'jetpack-ai-hub_init' );
	...
	jetpack_plugin_jetpack_ai_hub_preload_data();

	// Get all registered routes and menu items
	$menu_items = jetpack_plugin_get_jetpack_ai_hub_menu_items();
	$routes = jetpack_plugin_get_jetpack_ai_hub_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'jetpack-ai-hub-prerequisites', '', $asset['dependencies'], $asset['version'], true );

		// Add inline script to initialize the app
		$init_modules = ["@jetpack-plugin/init"];
		wp_add_inline_script(
			'jetpack-ai-hub-prerequisites',
			sprintf(
		...
		wp_register_style( 'jetpack-ai-hub-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for jetpack-ai-hub module
		...
		$boot_dependencies = apply_filters(
			'jetpack-ai-hub_boot_dependencies',
			$boot_dependencies
		);
		...
		wp_enqueue_script( 'jetpack-ai-hub-prerequisites' );
		wp_enqueue_script_module( 'jetpack-ai-hub' );
		wp_enqueue_style( 'jetpack-ai-hub-prerequisites' );
	}
	...
	<body class="jetpack-ai-hub no-js">
	<?php
	// BEGIN see wp-admin/admin-header.php
	?>
	...
	?>
		...
		<div id="jetpack-ai-hub-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
 ...
 */
function jetpack_plugin_jetpack_ai_hub_intercept_render() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'jetpack-ai-hub' === $_GET['page'] ) {
		...
		jetpack_plugin_jetpack_ai_hub_render_page();
		exit;
	}
}

// Hook the interceptor to admin_init
add_action( 'admin_init', 'jetpack_plugin_jetpack_ai_hub_intercept_render' );
