...
// Global storage for jetpack-settings-dashboard routes and menu items
global $jetpack_plugin_jetpack_settings_dashboard_routes, $jetpack_plugin_jetpack_settings_dashboard_menu_items;
$jetpack_plugin_jetpack_settings_dashboard_routes     = array();
$jetpack_plugin_jetpack_settings_dashboard_menu_items = array();

/**
 * Register a route for the jetpack-settings-dashboard page.
 *
 ...
 */
function jetpack_plugin_register_jetpack_settings_dashboard_route( $path, $content_module = null, $route_module = null ) {
	global $jetpack_plugin_jetpack_settings_dashboard_routes;
	...
	$jetpack_plugin_jetpack_settings_dashboard_routes[] = $route;
}

/**
 * Register a menu item for the jetpack-settings-dashboard page.
 *
 ...
 */
function jetpack_plugin_register_jetpack_settings_dashboard_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $jetpack_plugin_jetpack_settings_dashboard_menu_items;
	...
	$jetpack_plugin_jetpack_settings_dashboard_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the jetpack-settings-dashboard page.
 *
 * @return array Array of route objects.
 */
function jetpack_plugin_get_jetpack_settings_dashboard_routes() {
	global $jetpack_plugin_jetpack_settings_dashboard_routes;
	return $jetpack_plugin_jetpack_settings_dashboard_routes ?? array();
}

/**
 * Get all registered menu items for the jetpack-settings-dashboard page.
 *
 * @return array Array of menu item objects.
 */
function jetpack_plugin_get_jetpack_settings_dashboard_menu_items() {
	global $jetpack_plugin_jetpack_settings_dashboard_menu_items;
	return $jetpack_plugin_jetpack_settings_dashboard_menu_items ?? array();
}

/**
 * Preload REST API data for the jetpack-settings-dashboard page.
 * Automatically called during page rendering.
 */
function jetpack_plugin_jetpack_settings_dashboard_preload_data() {
...
/**
 * Render the jetpack-settings-dashboard page.
 * Call this function from add_menu_page or add_submenu_page.
 */
function jetpack_plugin_jetpack_settings_dashboard_render_page() {
	...
	/**
	 * Fires when the jetpack-settings-dashboard page is initialized so extensions can register routes and menu items.
	 */
	do_action( 'jetpack-settings-dashboard_init' );
	...
	jetpack_plugin_jetpack_settings_dashboard_preload_data();

	// Get all registered routes and menu items
	$menu_items = jetpack_plugin_get_jetpack_settings_dashboard_menu_items();
	$routes = jetpack_plugin_get_jetpack_settings_dashboard_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_style( 'jetpack-settings-dashboard-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for jetpack-settings-dashboard module
		 ...
		 * jetpack-settings-dashboard page.
		 *
		...
		$boot_dependencies = apply_filters(
			'jetpack-settings-dashboard_boot_dependencies',
			$boot_dependencies
		);
		...
		wp_register_script_module(
			...
			$build_constants['build_url'] . 'pages/jetpack-settings-dashboard/loader.js',
			$boot_dependencies
		);

		// Enqueue the boot scripts and styles
		wp_enqueue_script( 'jetpack-settings-dashboard-prerequisites' );
		wp_enqueue_script_module( 'jetpack-settings-dashboard' );
		wp_enqueue_style( 'jetpack-settings-dashboard-prerequisites' );
	}
	...
	$hook_suffix = 'jetpack-settings-dashboard';
	...
	<body class="jetpack-settings-dashboard no-js">
	<?php
	// BEGIN see wp-admin/admin-header.php
	?>
	...
	?>
		...
		<div id="jetpack-settings-dashboard-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
 ...
 */
function jetpack_plugin_jetpack_settings_dashboard_intercept_render() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'jetpack-settings-dashboard' === $_GET['page'] ) {
		...
		jetpack_plugin_jetpack_settings_dashboard_render_page();
		exit;
	}
}

// Hook the interceptor to admin_init
add_action( 'admin_init', 'jetpack_plugin_jetpack_settings_dashboard_intercept_render' );
