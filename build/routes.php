...
foreach ( $routes_by_page as $page_slug => $page_routes ) {
	...
	$global_name = 'jetpack_plugin_' . $page_slug_underscore . '_routes_data';
 ...
 */
function jetpack_plugin_register_page_routes( $page_routes, $register_function_name ) {
	...
	foreach ( $page_routes as $route ) {
		...
		if ( $route['has_content'] ) {
			...
			if ( file_exists( $content_asset_path ) ) {
				...
				$content_handle = 'jetpack-plugin/routes/' . $route['name'] . '/content';
				$extension = '.min.js';
		...
		if ( $route['has_route'] ) {
			...
			if ( file_exists( $route_asset_path ) ) {
				...
				$route_handle = 'jetpack-plugin/routes/' . $route['name'] . '/route';
				$extension = '.min.js';
				// Deregister first to override any previously registered version
				// (e.g., Core's default modules when running as a plugin).
...
// Page-specific route registration functions for jetpack-ai-hub
/**
 * Register routes for jetpack-ai-hub page (full-page mode).
 */
function jetpack_plugin_register_jetpack_ai_hub_page_routes() {
	global $jetpack_plugin_jetpack_ai_hub_routes_data;
	jetpack_plugin_register_page_routes( $jetpack_plugin_jetpack_ai_hub_routes_data, 'jetpack_plugin_register_jetpack_ai_hub_route' );
}
add_action( 'jetpack-ai-hub_init', 'jetpack_plugin_register_jetpack_ai_hub_page_routes' );

/**
 * Register routes for jetpack-ai-hub page (wp-admin mode).
 */
function jetpack_plugin_register_jetpack_ai_hub_wp_admin_page_routes() {
	global $jetpack_plugin_jetpack_ai_hub_routes_data;
	jetpack_plugin_register_page_routes( $jetpack_plugin_jetpack_ai_hub_routes_data, 'jetpack_plugin_register_jetpack_ai_hub_wp_admin_route' );
}
